    ...
    rows.each_with_index do |row, i|
      unless i == 0
        ...
        if row[1].include?("D/ST")
          ...
          name = p_info.first
          nfl_team = ""
          position = p_info.second
        else
          p_info = row[1].split ","
          name = p_info.first
          nfl_team = p_info.second.split.first
          position = p_info.second.split.second
        end

        p = Player.where(team_id: id, name: name, nfl_team: nfl_team, position: position).first
